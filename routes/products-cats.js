...
exports.get = function(req, res) {
    async.waterfall([
        function(callback){
            var linkArray =  req.params.id;
            var aliasArray = linkArray.split('_');
            var alias = aliasArray[0];
            console.log(alias);
            Alias.findOne({resourceAlias: alias}, function(err, alias){
                if (!alias){
                    console.log('Такого url не существует');
                    return res.render('../views/404');
                } else {
                    var resourceId = alias.resourceId;
                    var resourceType = alias.resourceType;
                }
                callback(null, resourceId, resourceType);
            });
        },
